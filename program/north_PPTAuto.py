# -*- coding: utf-8 -*-
"""
Created on Wed Dec 13 15:07:57 2023
...
# 程式執行位置
path = rf"C:\Users\User\Desktop\North_auto\{format_date}"
# 今日檔案位置
ppt_path = rf"C:\Users\User\Desktop\North_auto\{format_date}\{format_date}-0900-AM-常時分析報告.pptx"
# 昨日檔案位置
    ...
    for shape in slide.shapes:
        if shape.shape_type == MSO_SHAPE_TYPE.TABLE:
            for i in range(start, end):
                ...
                if i == start:
                    Date_String = f"{now_month}/{now_day}\n(今)"
                elif i == (start + 1):
                    Date_String = f"{next_month}/{next_day}\n(明)"
                elif i == (start + 2):
                    Date_String = f"{after_tomorrow_month}/{after_tomorrow_day}\n(後)"
                slide_text_box.text = ""
                font(slide_text_box, Date_String, RGB, size)
                
                
# 更新表格內日期
def sixth_slide_table_update_date(slide, start, end, RGB, size):
    for shape in slide.shapes:
        if shape.shape_type == MSO_SHAPE_TYPE.TABLE:
            for i in range(start, end):
                slide_text_box = shape.table.cell(0, i)
...
StreamLine_pattern = r"\d{1,2}月\d{1,2}日 08:00 700-850hPa 平均駛流場圖"
...
update_date(second_slide, Satellite_pattern, f"{now_month}月{now_day}日 08:00 衛星雲圖", RGBColor(12,51,115), 18)
update_date(thrid_slide, StreamLine_pattern, f"{now_month}月{now_day}日 08:00 700-850hPa 平均駛流場圖", RGBColor(12,51,115), 18)
...
update_date(sixth_slide, today_QPF_pattern, f"今({now_day})日", RGBColor(12,51,115), 18)
update_date(sixth_slide, tomorrow_QPF_pattern, f"明({next_day})日", RGBColor(12,51,115), 18)
update_date(sixth_slide, day_after_tomorrow_QPF_pattern, f"後({after_tomorrow_day})日", RGBColor(12,51,115), 18)
sixth_slide_table_update_date(sixth_slide, 1, 4, RGBColor(00,00,00), 18)
table_update_date(seventh_slide, 4, 7, RGBColor(00,00,00), 16)
table_update_date(eighth_slide, 4, 7, RGBColor(00,00,00), 16)
table_update_date(ninth_slide, 4, 7, RGBColor(00,00,00), 16)
...
if os.path.exists(rf"C:\Users\User\Desktop\North_auto\{format_date}\E_08_image.png"):
    change_img(forth_slide, "E_08_image.png", 6784343)
else:
    change_img(forth_slide, "E_image.png", 6784343)

change_img(forth_slide, "E_yday_image.png", 1276959)
change_img(fifth_slide, "cropped_6_06QPF.png", 428261)
change_img(fifth_slide, "cropped_6_12QPF.png", 3283377)
change_img(fifth_slide, "cropped_6_18QPF.png", 6152135)
change_img(fifth_slide, "cropped_6_24QPF.png", 9005248)
...
try:
    ...
    prs.save(f'{format_date}-0900-AM-常時分析報告.pptx')
    write_txt("簡報儲存成功")
    print("成功儲存簡報")
